...
class ShowImage(QMainWindow):
    def __init__(self):
        ...
        ui_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'A6.ui')
        loadUi(ui_path, self) 
    ...
    # FUNGSI OPERASI TITIK (KUMULATIF A3 - A6)
    ...
    def contrastStretching(self):
        if self.image is not None:
            ...
            # Cari nilai piksel terkecil (rmin) dan terbesar (rmax)
            rmin = np.min(img)
            rmax = np.max(img)
            ...
            for i in range(h):
                for j in range(w):
                    ...
                    # Mencegah error pembagian dengan nol kalau gambarnya 1 warna doang
                    if rmax != rmin:
