...
class ShowImage(QMainWindow):
    def __init__(self):
        ...
        ui_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'A8.ui')
        loadUi(ui_path, self) 
        ...
        # --- KONEKSIKAN MENU BAR KE FUNGSI (KUMULATIF) ---
        ...
        # Kabel Baru untuk A8 (Final Boss)
    ...
    # KUMPULAN FUNGSI OPERASI TITIK (KUMULATIF A3 - A8)
    ...
    def binerImage(self):
        if self.image is not None:
            ...
            h, w = img.shape[:2]
            threshold = 128 # Nilai tengah untuk penentuan hitam/putih
            ...
            for i in range(h):
                for j in range(w):
                    ...
                    # Rumus Citra Biner
                    if a > threshold:
                        img[i, j] = 255 # Jadi Putih
                    else:
                        img[i, j] = 0   # Jadi Hitam
