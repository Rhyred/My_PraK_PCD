...
class ShowImage(QMainWindow):
    def __init__(self):
        ...
        ui_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'A10.ui')
        loadUi(ui_path, self) 
    ...
    def rgbHistogram(self):
        if self.image is not None:
            # Cek apakah gambar punya warna (3 channel)
            if len(self.image.shape) == 3:
                # OpenCV baca warna dengan format BGR (Blue, Green, Red)
                color = ('b', 'g', 'r')
                
                plt.figure("Histogram RGB")
                for i, col in enumerate(color):
                    # Hitung dan plot histogram per warna
